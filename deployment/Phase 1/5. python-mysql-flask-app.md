...
Since I have installed mysql. I just imported the sql file to the database server

Create database first
```bash
sudo mysql -u root -p
```
Then enter password, and create database
```mysql
CREATE DATABASE vanciklin;
```
Imported database
```bash
sudo mysql vanciklin < /projects/vanciklin/vanciklin.sql
```
To check database and tables successfully imported
